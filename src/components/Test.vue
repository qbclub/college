						...
						<v-col v-for="(answer, index) in current_question.answers" :key="index">
							<AnswerInput v-if="current_question.type === 'input'" :answer="answer"
								...
								<template v-slot:input>
									<input
										:class="{ border: true, 'bg-green-lighten-3': current_question.entered && compareInput(answer.value, answer.answer), 'bg-red-lighten-3': current_question.entered && !compareInput(answer.value, answer.answer) }"
										type="text" v-model="test.questions[current_step - 1].answers[index].value"
										:disabled="current_question.entered">
								</template>
							</AnswerInput>
